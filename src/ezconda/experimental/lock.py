...
def write_lock_file(env_name) -> None:
    ...
    """
    typer.secho(f"Writing lock file...", fg=typer.colors.YELLOW)
    # generate lock file
...
def read_lock_file_and_install(lock_file: Path, env_name: str, verbose: bool) -> None:
    ...
    _ = run_command(
        Commands.CREATE, "-n", env_name, use_exception_handler=True
    )
    typer.secho(f"Reading lock file...", fg=typer.colors.YELLOW)
    ...
    for channel in channels:
        ...
        typer.secho(f"Collecting packages for channel : {channel}", fg=typer.colors.YELLOW)
        pvb = [
            f"{d['name']}={d['version']}={d['build_string']}"
            for d in complete_spec
            if d["channel"] == channel
        ]
        typer.secho(f"Installing packages for channel : {channel}", fg=typer.colors.YELLOW)
        stdout, stderr, exit_code = run_command(Commands.INSTALL, "-n", env_name, *pvb, "-c", channel)
        ...
        if exit_code != 0:
            ...
            raise typer.Exit()
        typer.secho(f"Installed all packages from channel : {channel}", fg=typer.colors.BRIGHT_GREEN)
    

    typer.secho(f"Installed all dependencies from '{lock_file}'!", fg=typer.colors.BRIGHT_GREEN)